...
On a phone the desktop becomes a map: every window is a labelled box (*Terminal ~ ❯*, *Chromium omarchy.org/why*, *Neovim*, *Files*) in the real dwindle layout, so the tour's moves stay visible on a 390px screen — and because a portrait screen is taller than wide, the first split is stacked and Super + J puts the windows side by side, which the coach says. The tour ends with the browser full screen: the why page, with the *All done* notification on top. After that a tapped box opens full screen and the current workspace number in the bar brings the map back. Boxes need a narrow *and* touch-driven screen; a narrow desktop pane keeps real windows.
