...
On a phone the desktop becomes a map: every window is a labelled box (*Terminal ~ ❯*, *Chromium omarchy.org/why*, *Neovim*, *Files*) in the real dwindle layout, so the tour's moves stay visible on a 390px screen — and because a portrait screen is taller than wide, the first split is stacked and Super + J puts the windows side by side, which the coach says. The tour ends with the other windows closed and the browser full screen: the why page, with the *All done* notification on top. The bar's Omarchy icon (waybar's `custom/omarchy`, which runs `omarchy-menu` on click) is the way into the menu, the cheat sheet and the theme switcher there. The welcome notification isn't shown on phones. Boxes need a narrow *and* touch-driven screen; a narrow desktop pane keeps real windows.

Review shortcuts: `?skip=1`, `?step=N`, `?theme=name`, `?phone=1` (the phone map at any narrow width).
